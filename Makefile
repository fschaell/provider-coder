...
PROJECT_REPO := github.com/fschaell/$(PROJECT_NAME)
...
IMAGES = provider-coderworkspaces
...
XPKGS = provider-coderworkspaces
-include build/makelib/xpkg.mk

# NOTE(hasheddan): we force image building to happen prior to xpkg build so that
# we ensure image is present in daemon.
xpkg.build.provider-coderworkspaces: do.build.images
...
dev: $(KIND) $(KUBECTL)
	...
	@$(INFO) Installing Provider CoderWorkspaces CRDs
	@$(KUBECTL) apply -R -f package/crds
	@$(INFO) Starting Provider CoderWorkspaces controllers
...
# This target prepares repo for your provider by replacing all "coderworkspaces"
